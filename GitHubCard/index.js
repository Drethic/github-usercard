...
axiosGet(`${baseUrl}/${user}`).then(response => cards.appendChild(cardCreator(response.data)));
axiosGet(`${baseUrl}/${user}/followers`)
  .then(response => response.data.map((follower) => {
    axiosGet(`${baseUrl}/${follower.login}`)
      .then(followerResponse => {
        cards.appendChild(cardCreator(followerResponse.data));
      })
  }));
